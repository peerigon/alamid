...
patterns = {    // functions are used, so the regular expression object is always new
    ...
    servicesPath: function servicesPath() { return /^services\/.+/g; },
    validatorsPath: function validatorsPath() { return /^validators\/.+/g; }
};
...
filters = {
    ...
    onlyServicesPath: function onlyServicesPath(path) {
        return patterns.servicesPath().test(path);
    },
    onlyValidatorsPath: function onlyValidatorsPath(path) {
        return patterns.validatorsPath().test(path);
    }
};
