...
function init(server) {
    ...
    server.use("/", function onStaticRequestHandler(req, res, next) {
        applyMiddleware(onStaticRequest, req, res, function() {
            console.log("on static request NEXT");
            next();
        });
    });

    /*
    //not needed anymore because we serve the bootstrap via static file server
