...
function handleRequest(req, callback) {
    ...
    if (typeof(res.once) === "function") {
        ...
        res.once("end", function (err) {
            ...
            log.info(method.toUpperCase() + " " + req.getRawPath() + " " + " (" + transportType + ") " + res.getStatusCode() + " " + res.getStatus().toUpperCase() + " - " + (new Date() - startTime) + "ms (middleware-ended)");
            callback(err, req, res);
        });
    }

    log.debug(method.toUpperCase() + " " + req.getRawPath() + " (" + transportType + ")");
    ...
    dispatch(req, res, function (err) {
        ...
        log.info(method.toUpperCase() + " " + req.getRawPath() + " " + " (" + transportType + ") " + res.getStatusCode() + " " + res.getStatus().toUpperCase() + " - " + (new Date() - startTime) + "ms");
