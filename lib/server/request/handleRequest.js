...
function handleRequest(req, callback) {
    ...
    if(isValidRequest) {
        ...
        log.info(method.toUpperCase() + " " + req.getRawPath() + " (" + transport + ")");

        dispatch(req, res, function(err) {
            log.debug(method.toUpperCase() + " " + req.getRawPath() + " " + " (" + transport + ") " + res.getStatusCode() + " " + res.getStatus().toUpperCase() + " - " + (new Date() - startTime) + "ms");
