...
function publishModel(req, res, next) {
    ...
    if(status === "success" && type === "service" && method !== "read") {
        ...
        if(method === "create") {
            ...
            socket.broadcast.to(room).emit("remoteCreate", model.getUrl(), model.getParentIds(), model.toObject());
        }

        //update
        if(method === "update") {
            //should have ID and everything baked in
            socket.broadcast.to(room).emit("remoteUpdate", model.getUrl(), model.getParentIds(), model.toObject());
        }
