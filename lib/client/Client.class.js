...
var Client = EventEmitter.extend("Client", {
     ...
     *
     * If the handler is a string it will be interpreted as pageUrl calling client.mainPage.changePage() with that given pageURL.
     *
        ...
        handlers = handlers.map(function prepareRouteHandler(handler) {
            ...
            if (value(handler).typeOf(String)) {
                ...
                handler = function handleRoute(ctx) {
                    self.mainPage.changePage(pageUrl, ctx);
                };
    ...
    start: function (pageJSOptions) {
        ...
        if (!mainPage) {
            ...
            this.mainPage = new MainPage(initialContext, document);
        }
...
function addDefaultRouteHandler(self) {
    pageJS("*", function (ctx) {
        self.mainPage.changePage(ctx.pathname, ctx);
    });
}
