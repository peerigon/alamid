...
var ViewCollection = Displayable.extend("ViewCollection", {
    ...
    constructor: function (ViewClass, collectionTemplate) {
        ...
        viewCollectionNodes = this.find("[data-node='" + this.viewsNode  + "']");
        hasOneDataNode = viewCollectionNodes.length === 1;

        if (hasOneDataNode === false) {
            throw new Error(
                "(alamid) Cannot construct ViewCollection: " +
                "Given template has no or more than one node with [data-node='" + this.viewsNode +
    ...
    dispose: function () {
        ...
        this._removeModelsListeners();
        this._super();
        // child views should be disposed by Displayables regular dispose, as they are added by append/prepend
        this._views = null;
    ...
    _addViews: function (views, index) {
        ...
        //.prepend() prepends according to current first child of ViewCollection.
        ...
        _(views).each(function _addView(view) {
            ...
            if (prepend === true) {
                self.prepend(view).at("views");
            } else {
                self.append(view).at("views");
            }
    ...
    _addModelsListeners: function () {
        this._models.on("add", this._onAdd);
        this._models.on("remove", this._onRemove);
        this._models.on("sort", this._onSort);
    },
