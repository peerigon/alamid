...
var Displayable = EventEmitter.extend("Displayable", {
    ...
    emit: function () {
        ...
        if (eventName === "document") {
            ...
            _(this._children).each(emitDocumentOnChild);
        }
    ...
    if (!root) {
        ...
        for (i = 0; i < children.length; i++) {
            ...
            if (node.nodeType === Node.ELEMENT_NODE) {
                if (root) {
                    throw new Error("(alamid) Error: Template of '" + self.constructor.name + "' has more than one root node.");
                }
                root = node;
            }
        }
        if (!root) {
            throw new Error("(alamid) Error: Could not find a root node in template of '" + self.constructor.name + "'.");
        }
...
function emitDocumentOnChild(child) {
