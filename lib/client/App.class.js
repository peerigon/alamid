...
var _ = require("underscore"),
    ...
    subscribeModelHandler = require("./subscribeModelHandler.js");
...
var App = new Class("App", {
    ...
    init: function (MainPageClass) {
        ...
        if (value(MainPageClass).notInstanceOf(Function)) {
        ...
        _(handlers).each(function prepareHandler(handlerToPrepare, index) {
            ...
            if (isDev) {
                ...
                preparedHandler = function debugDispatchRoute(ctx, next) {
                    console.log("dispatching:" + route, ctx);
    ...
    /**
     * @param @param {!String|Function} handler
     * @private
     */
    _prepareRouteHandler: function (handler) {
        ...
        if (value(handler).instanceOf(String)) {
            pageURL = handler;
            handler = function handleRoute(ctx) {
                self.changePage(pageURL, ctx);
            };
        } else if (value(handler).notInstanceOf(Function)) {
    ...
    __getIndexOfTransitionPage: function (pageURLs) {
        ...
        _(pageURLs).find(function findTransitionPageURL(pageURL, i) {
            ...
            return typeof PageClass !== "function" || value(page).notInstanceOf(PageClass);
        });
