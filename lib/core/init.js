var settings = require('./settings.js'),
    ...
    prepareHTML = require('../tools/prepareHTML.js'),
    toSrc = require('toSrc'),
    async = require('async'),
    fs = require('fs');

var pathModifier = paths.getPathModifier(
        ['noServerFiles'],           // filters
        ['nodeModules'] // modifier
    ),
    patternLeft = '<script type="text/javascript">',
    patternRight = '</script>',
    availableServices = {},
    availableValidators = {};

function createSettings() {
    var newSettings = fs.readFileSync(paths.appConfig, 'utf8');

    try {
        newSettings = JSON.parse(newSettings);
    } catch (err) {
        throw new Error('Error while parsing the config.json\n' + err.message);
    }
...
function writeClientInit(callback) {
    ...
    function writeInitHTML() {
        ...
        initHTML = prepareHTML(initHTML, settings.basePath);
        writeScripts();
    ...
    moduleFinder.walkSync(paths.appServices + '/pages');    // all client page services shall be available on startup
