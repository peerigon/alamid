...
function resolveFilename(filename, callback) {
    ...
    //ALAMID MODULES
    //SERVER
    if(realModulePath.indexOf("alamid/lib/server") !== -1 && /\.server(\.class)?\.js$/i.test(realModulePath) === true) {
    ...
    //CORE
    if(realModulePath.indexOf("alamid/lib/core") !== -1 && /\.server(\.class)?\.js$/i.test(realModulePath) === true) {
