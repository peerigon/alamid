...
function getPage(uri, onPageLoaded, onDataLoaded) {
    ...
    positionOfQuestionMark = uri.search(/\?/);
    if (positionOfQuestionMark === -1) {
        params = {};
    } else {
        params = uri.substr(positionOfQuestionMark + 1);
        uri = uri.substr(0, positionOfQuestionMark);
        params = domAdapter.QueryParse(params);
    }
    uri = settings.baseURL + '/pages/' + uri;
    ...
    try {
        ...
        loadScript(, doPageLoadedCallback);
    }
