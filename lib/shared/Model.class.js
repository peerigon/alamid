...
module.exports = new Class({
    ...
    init: function init(id) {
        ...
        //add if not! because new is also used by .findById();
        if(id !== null) {
            var cachedModel = modelLoader.get(url, id);
            if(cachedModel === null) {
                //How to get the real Instance?
                modelLoader.add(this);
            }
        }
    },
        ...
        var caster = {
            ...
            Number : {
                Date : function(value) {
                    ...
                    //never invalid date just start of timestamp
                    return resDate;
                },
    ...
    __callService : function(serviceFunction, model, callback) {
    ...
    fetch: function fetch(callback) {
        ...
        this.__callService(service.read, this.Instance, onServiceResponse);
    },
    save: function save(callback) {
        ...
        function onServiceResponse(response) {
            ...
            if (!err) {
                //TODO what happens if instance was created via "new Model"?
                //at the moment the instance would be overwritten/reLinked to new instance
                //instance has not been part of the registry before -> store new instance
                if(method === "create") {
                    modelLoader.add(self.Instance);
                }
                self.emit("save");
            }
            callback(err);
        ...
        this.__callService(service[method], this.Instance, onServiceResponse);
    },
    destroy: function (callback) {
        ...
        this.__callService(service.delete, this.Instance, onServiceResponse);
    },
    ...
    $findById : function(ModelClass, id, callback) {
        var instance = modelLoader.get(ModelClass.url, id);

        //if we don't find an instance, we add one!
        if(instance === null) {
            instance = new ModelClass(id);
            //instance are now being added in Model-construtor
            //modelLoader.add(instance);
        }
            ...
            for(arrayElem in modelDataArray) {
                if(modelDataArray.hasOwnProperty(arrayElem)){
                    ...
                    if(activeModelData.id !== undefined) {
                        ...
                        modelInstance = modelLoader.get(ModelClass.url, activeModelData.id);
                        //modelInstance = null;
                        if(modelInstance === null) {
                            ...
                            //console.log("modelInstance loaded from cache", modelInstance);
                            //console.log("normal instace", new ModelClass(activeModelData.id));
                        }
