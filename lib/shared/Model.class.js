...
module.exports = new Class({
    ...
    //_setSchema(schema)
     "_setDefaults": function (defaults) {
        ...
        this.__keys = [];
        for (key in defaults) {
            if (typeof key === 'object' || (typeof key === 'function' && acceptedFunctions.indexOf(key) === -1)) {
                throw new TypeError('A model can only contain native types.'
                    + '\nIf you want to save a reference, you should store the ID');
            }
            this.__keys.push(key); //all values that can be set!
        }
    ...
    "validate": function validate(clientOnly, callback) {
        ...
        //each model instance has it's own schema (singelton)
