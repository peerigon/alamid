...
var value = require("value"),
    ...
    AttributeSetter = require("./helpers/AttributeSetter.js");
...
var Model = EventEmitter.extend("Model", Junction, {
    ...
    setter: function (key, newValue) {
        ...
        // If there is a signal, we use that because it has its own setters
        ...
        configureAttributeSetter(attributeSetter, this, key);
    
        this._attributes[key] = attributeSetter(newValue);
    },

    /**
     * Returns a signal for the given attribute.
     *
    ...
    signal: function (key) {
        ...
        if (needsToBeConfigured) {
            signal.setter = configureAttributeSetter(new AttributeSetter(), this, key);
            signal.notify(onSignalChange.bind(this));
        }
     ...
     * is not removed, but resetted to its default value.
     * You can pass many keys at once to unset multiple keys.
...
function configureAttributeSetter(attributeSetter, model, key) {
    attributeSetter.expectedType = model._types? model._types[key] : null;
    attributeSetter.casting = model._casting;
    attributeSetter.key = key;
    attributeSetter.customSetters = model._setters? model._setters[key] : null;
