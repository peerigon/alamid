...
module.exports = new Class({
    ...
    init: function init(id) {
        ...
        //console.log("url", this.Instance.constructor.url);
    ...
    getRequestUrl : function() {
        ...
        if(this.__url === undefined) {
            throw new Error("(alamid) No Url set for Model.");
        }

        //nothing to merge here
        if(this.__parentIds === null) {
            return this.__url;
        }

        var urlSplit = this.__url.split("/"),
            resArray = [];

        for(var i = 0; i < urlSplit.length; i++) {
            resArray.push(urlSplit[i]);
            //is an id defined?
            if(this.__parentIds[urlSplit[i]] !== undefined) {
                resArray.push(this.__parentIds[urlSplit[i]]);
            }
        }
    ...
    validate: function (callback) {
        ...
        if(schema !== null && sharedSchema !== null) {
            validate(sharedSchema, schema, this.__url, this.get(), fullValidation, callback);
        }
        else{
