var typeOf = require('./typeOf.js'),
    servicesModule = require('./services.js'),
    validatorsModule = require('./validators.js'),
    servicesAdapter = require('misc/servicesAdapter.js');

var acceptedFunctions = [Number, String, Boolean, Date],
    acceptedFunctionsLiteral = ['Number', 'String', 'Boolean', 'Date'];
...
var Class = {
    "init": function init(filename, defaults) {
        var key;
        ...
        if (this._validators === null) {
            this._service = servicesModule.getService(filename);
            this._validators = validatorsModule.getValidators(filename);
        }
    ...
    "validate": function validate(clientOnly, callback) {
        ...
        if ((clientOnly && !validators.client) ||
