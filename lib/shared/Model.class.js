...
var Model = EventEmitter.extend("Model", Junction, {
    ...
    /**
     * Get the internal object-representation of the model. The returned object contains id, ids
     * and all model-attributes. This is meant to be read-only because changes to this object will
     * not emit any change events.
     *
     * If options are supplied a copy of the internal object is returned.
     * 
     * Options example:
     * {
     *      exclude: ["secretField"],
    ...
    fetch: function (remote, callback) {
        ...
        serviceAdapter(this, "read", remote, function onServiceResponse(err, response) {
            ...
            if (cache && !cache.get(self.getId())) {
                cache.set(self.getId(), self);
            }
