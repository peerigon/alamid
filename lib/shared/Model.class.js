...
var Class = {
    ...
    "__init": function __init() {   // used by the init function and removeAll since they act similarly
        ...
        this._attributes = Changed.prototype = new Attributes();
    ...
    "_setDefaults": function (defaults) {
        ...
        if (this._defaults !== null) {
            throw new Error("You cannot set the defaults twice on a model");
        }
        for (key in defaults) {
    ...
    "get": function get(key) {
        ...
        if (arguments.length === 1) {
            result = this._changed[key];
            result = checkSingleResult(result);
        } else {
            result = {};
            for (i = 0, l = arguments.length; i < l; i++) {
                key = arguments[i];
                result[key] = checkSingleResult(this._changed[key]);
            }
        }
    ...
    "escape": function escape(key) {
        ...
        if (arguments.length === 1) {
            result = doEscape(this._changed[key]);
        } else {
            result = {};
            for (i = 0, l = arguments.length; i < l; i++) {
                key = arguments[i];
                result[key] = doEscape(this._changed[key]);
            }
        }
