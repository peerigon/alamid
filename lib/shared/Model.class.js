...
module.exports = new Class({
    ...
    setSchema: function (schema) {
        ...
        for (key in schema) {
            if(schema.hasOwnProperty(key)){
                fieldDefintion = schema[key];
                //determine supported types
                type = schemaHelpers.determineType(fieldDefintion);

                this.__schema[key] = {
                    type : type,
                    default : fieldDefintion.default,
                    validate : fieldDefintion.validate
                };

                this.__keys.push(key);

                if(fieldDefintion.default === undefined) {
                    fieldDefintion.default = null;
                }

                this.__defaults[key] = fieldDefintion.default;
            }
        }
    ...
    validate: function (validationType, callback) {
        //each model instance has it's own schema (singleton)
        //call functions on schema for validation
        var sch = this.schemaRef;

       validate(sch, this.get(), validationType, callback);
