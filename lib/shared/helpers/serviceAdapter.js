...
function processResponse(modelInstance, response) {
    ...
    if (modelData && value(modelData).notTypeOf(modelInstance.Class)) {
        modelInstance.update(modelData);    // this may throw an exception
    }
}
...
function serviceAdapter(modelInstance, method, remote, callback) {
    ...
    if (service && service[method] !== undefined) {
        serviceFunction = service[method];
    }
    ...
    if (env.isClient() && remote === true) {
        ...
        remoteServiceFunction = remoteService[method];
        //append to be used in service
        remote = remoteServiceFunction.bind(remoteService);
    }
    ...
    switch (method) {
        ...
        case "create" :
            callService.call(service, serviceFunction, remote, ids, modelInstance, onServiceResponse);
            break;
        case "read" :
            callService.call(service, serviceFunction, remote, ids, onServiceResponse);
            break;
        case "readCollection" :
            //direct callback here, no processing, because there is no instance
            callService.call(service, serviceFunction, remote, ids, params, callback);
            break;
        case "update" :
            callService.call(service, serviceFunction, remote, ids, modelInstance, onServiceResponse);
            break;
