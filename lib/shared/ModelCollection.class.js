...
var ModelCollection = new Class("ModelCollection", {
    ...
    remove: function (modelOrIndex) {
        ...
        if (typeof modelOrIndex === "number") {
            ...
            _(this.Super._getElements()).find(function findModelIndex(model, index) {
                if (modelOrIndex === model) {
                    removeIndex = index
                    return true;
                }
            });
        ...
        return this.Super.remove(removeIndex)
    ...
    dispose: function () {
        ...
        _(models).each(function removeAllAddedListeners(model) {
    ...
    _createDeleteListener: function (model) {
        ...
        function onDelete(err) {
            ...
            if(!err) {
                ...
                models = self.Super._getElements();

                _(models).find(function findModelInCollection(collectionModel, index) {

                    if (collectionModel === model) {
                        self.Super.remove(index);
                        //@TODO https://github.com/peerigon/alamid/issues/109
    ...
    _removeModelListeners: function (model) {
        ...
        var listenerModelIndex,
            listenerModels = this.__listeners.models;

        _(listenerModels).find(function findModelIndex(listenerModel, index) {
            if (listenerModel === model) {
                listenerModelIndex = index;
                return true;
            }
        });

        model.removeListener("change", this.__listeners.change[listenerModelIndex]);
        model.removeListener("delete", this.__listeners.change[listenerModelIndex]);
