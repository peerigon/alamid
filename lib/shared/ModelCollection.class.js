...
var ModelCollection = Collection.extend("ModelCollection", {
    ...
    _addModelListeners: function (model) {
        ...
        this._listeners.models.push(model);
        this._listeners.change.push(onChange);
        this._listeners.destroy.push(onDestroy);
    },
    ...
    _removeModelListeners: function (model) {
        var listenerModelIndex = _(this._listeners.models).indexOf(model);

        model.removeListener("change", this._listeners.change[listenerModelIndex]);
        model.removeListener("destroy", this._listeners.destroy[listenerModelIndex]);
    }
});
